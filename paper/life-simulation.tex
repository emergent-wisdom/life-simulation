...
\ewreviseddates{September 5, 2026}{September 19, 2026}
...
\begin{abstract}
\noindent Text, measurements, and actions reveal fragments of processes that
continue beyond each observation. Life Simulation proposes learning to
construct and refine numerical accounts of evolving worlds, across people,
economies, institutions, and environments. Construction begins with coarse
histories and progressively opens finer detail. World histories teach how
worlds develop; construction histories teach how to model them: which categories
to invent, where to open detail, and when to revise explanations. Trained
learners would then help construct their next training corpus. Conceptual
shares compare categories under declared questions; measured and inferred
empirical quantities retain units and uncertainty. The Meaning Model is the
shared representation connecting these histories with text, concepts, visual
tracks, and expressed understanding.

Narrative-world construction is the first controlled laboratory, not the scope
limit. \emph{The Book of Conditions} is a completed book built through this
method; its numerical dependence remains untested. Proposed tests ask whether numerical histories shape generation, whether
process histories improve prospective prediction beyond matched controls, and
whether learning from construction histories improves the next corpus and the
learner trained on it.
Success would motivate reconstructing existing books and historical accounts,
toward one connected representation of world history and all texts. This
includes discovering new processes and recovering quantitative histories not
explicitly recorded. Training aims to develop a process sensorium: a learned
ability to infer, track, and simulate change, supporting problem solving and
whole-life understanding during conversation.
...
circumstances. Under Entangled Alignment, the model acts as a Reader whose
interpretations begin with seven stable first-person ethical commitments, the
Core, throughout the corpus. The aim is to develop care alongside capability
and retain that orientation through self-improvement. Evidence, inference, and
invention remain distinct. Generative utility does not establish real-world
accuracy or alignment; learning and safety benefits remain to be demonstrated.
\end{abstract}
...
Text is what processes leave behind: a message, a ledger entry, or a measurement
records one moment of something that continues after the record ends. Life
Simulation aims to model those processes explicitly and to train AI on two
kinds of history. World histories teach how a world develops: what changes,
what interacts, and what consequences follow. Construction histories teach
how to model it: what to track, which categories to invent, where to open
detail, and when to revise an explanation. The second target matters as much
as the first. The learner does not merely receive better annotations; it
learns the work that produces better representations, and with them the
material for its own further training. A process reconstructed from text
remains an account to test, not access to the truth.
...
often-implicit conceptual comparisons explicit as learning
...
Life Simulation proposes teaching an AI to construct and improve numerical
accounts of a world, including the concepts and processes those accounts use.
The model would not only estimate values for predefined variables: it could
propose what to track, give its numbers a declared meaning, and construct
provisional histories of how those processes change and interact. A story
becomes a place to use and examine that account: do characters' actions and
larger events follow from the processes represented? A failure may call for
different values, a missing process, or a different way of dividing the
situation. Learning from these construction and repair decisions is intended
to improve the model's ability to build subsequent accounts, including
interpretations of existing texts. For these
...
units. Meaning Model supplies that representational contract.

The text and linked numerical histories then become training material.
The proposed contribution is to train a process sensorium across the corpus
(Section~\ref{sec:process-sensorium}): a learned capacity to infer, maintain,
and revise explicit numerical accounts of the processes underlying text and
observations. The intended scale is one connected, revisable account of world
history and its texts, not a separate model for each book. Linking partial and
potentially biased sources could let evidence from one constrain or revise
interpretations of another, supporting reconstruction of their shared reality
while retaining uncertainty and competing explanations. Within this shared
representation, process accounts are decomposed across concepts by declared
questions and across time by progressive resolution; different perspectives and
factual, fictional, and counterfactual histories remain distinct. World
histories teach how those processes develop; construction
histories teach which distinctions to introduce and how to repair their
representations. The learned capability would then help construct the next
training corpus, re-estimating values, adding useful distinctions, resolving
temporal detail, and repairing earlier accounts. Section~\ref{sec:related}
states the three testable hypotheses and one research aim.

Here the \emph{modeler} is the person or AI system constructing, interpreting,
or revising an account; \emph{constructor} names its role when proposing
changes. The continuing Reader is the particular modeler role used for
Core-guided interpretation in the recommended alignment treatment.
...
The method handles complexity through recursive decomposition: each process
can be opened into interacting finer processes, which can themselves be opened
further across conceptual and temporal scales.
...
The first test is generative: do numerical histories actually guide coherent
stories, game worlds, and character behavior? Section~\ref{sec:narrative-laboratory}
makes this test concrete before large-scale source annotation.
...
work, operational planning, or exploration without first becoming a training
corpus. The other is to learn from the histories those worlds expose. A learner would not only predict
...
Meaning Model and Life Simulation have different responsibilities during the
same construction. Meaning Model supplies stable identities, attributed
accounts, and coordinated revision of world records, concepts, descriptions,
understanding, and prose~\cite{westerbergMeaning2026}. Their shared addresses
and declared dependencies identify what a change puts in question without
merging their authority. Life Simulation develops how to advance process
histories, search for dynamics, infer hidden paths, and learn from synchronized
streams and later corrections. Its proposals still pass the world interface's
acceptance and revision rules. Meaning Model is the selected interface, not a
mandatory encoding: another representation could be tested if it supplied
equivalent identity, time, provenance, and uncertainty contracts.
...
The bootstrap begins with coarse worlds, not a model already able to
reconstruct the whole corpus. Forward construction supplies controlled examples
for inverse learning; failed interpretations reveal distinctions that later
construction should test. Both routes can improve before either is
comprehensive. Progressively modeling a source is therefore productive work,
not merely preparation for a dataset assembled once the model is already
capable. A constructed motive can be chosen, while a reconstructed motive
must remain uncertain where the source does not decide. Generative tests,
source checks, and held-out learning tests therefore have separate jobs.
...
Meaning Model paper as a completed manuscript with linked numerical process
descriptions and a partial construction witness. It shows that the authoring
approach can produce a finished narrative. Independent read-back,
numerical-dependence tests, the matched literary comparison, and learning
benefits remain prospective. Section~\ref{sec:narrative-laboratory} explains the
controlled advantage of authored worlds and the further learning tests.
Life Simulation can learn from accepted histories or propose continuations
during construction; Meaning Model still governs their acceptance and rendering.
...
instantiate. What unifies the architecture is a shared representational
framework for worlds and people, concept definitions, documents, and accounts
of the modeler's own activity, including Solvers and their executions
(Section~\ref{sec:corpus-model}). Concepts can
themselves be grounded in process models expressed in the same grammar as the
situations they describe. These linked accounts retain distinct roles and
evidence status while supporting coarse-to-fine construction with explicit
revision, joint prediction of native text and process state, and learning how
to construct and repair the representations themselves. Stable identities
connect records across revisions; Sema identifies exact definition and
grounding versions. The proposed training loop would return that learned
capacity to the construction of subsequent training material. Existing
construction artifacts demonstrate parts of the workflow, not this end-to-end
learned capability. Section~\ref{sec:related} names the precedents for each
part.

Temporal Hindsight Learning is a supported training approach, not
...
independent evidence. Section~\ref{sec:learning} develops the two routes to
improving this capability: learning from histories and improving tools and
their use.
...
Section~\ref{sec:temporal-object} defines the evolving world, the process
sensorium, and the connected corpus model; Section~\ref{sec:meaning-interface}
states the representation contract. Evaluation gates and common controls
precede adaptive resolution, candidate dynamics, and the primary narrative
tests. Section~\ref{sec:learning} develops learning from world and construction
histories. Section~\ref{sec:problem-solving-events} treats problem solving as
part of the same world, including simulated implementation and its effects,
and agents rehearsing decisions from inside a character's limited perspective.
The two bounded studies in Section~\ref{sec:component-experiments} isolate
process supervision and repair learning. Later sections address real-person
evidence, alignment, applications, and related work. Component tests need
not wait for implementation of the full program.
...
The object below separates a world's evolution from what a learner can
observe. Its projections supply the joint learning task.
...
A value need not be measured or already known to be accurate before it can
serve a modeling purpose. The constructor may begin with an authored value or
a provisional estimate, provided its meaning and provenance are explicit.
Such values allow the account's consequences to be explored and its
assumptions challenged; revision may change both the values and the processes
being represented. In a fictional world, the tests concern consistency and
whether the numerical histories actually influence the generated behavior.
In an account of reality, observations withheld from construction must
additionally test whether the estimates and their consequences are accurate.
Learning from successive constructions and corrections is intended to improve
this modeling practice; internal coherence alone does not establish that
improvement.
...
interpretively changes.

Anticipation belongs to a named perspective and cutoff. Before the outcome, a
...
More capable systems, potentially including artificial superintelligence,
may develop distinctions beyond categories humans can readily understand.
Where a fine-to-coarse preservation contract is declared, the finer records
must reconstruct protected readable answers within its tolerances or revise
those commitments explicitly. A Cut's remainder retains unallocated share
under its question. This preserves a level people can inspect without claiming
that unfamiliar detail is correct; held-out predictions must test its value.
...
operative model, self-report, and observer hypotheses are separate objects.
For an authored character, simulation can use its authored operative account.
Where the operative organization is unknown, simulation uses a declared
estimate of it, retaining its attribution and version. A simulated decision
for $a$ receives only records accessible through the selected account and
actually observed or delivered by the cutoff. Mere
...
Using Meaning Model's optional modeler representation in
``World, Understanding, and Story''~\cite{westerbergMeaning2026},
the modeler can also have a represented account: a Thing with its
own identity, lifecycle, and inner-process root on the reading or construction
clock. Its changing attention, uncertainty, concern, and intentions may be
Event-processes; Understanding Nodes record its expressed questions,
interpretations, and revisions. For a character or a modeler, either form,
both, or neither may be used. A reaction node can address a world Event,
concept model, or passage without requiring a detailed process account.
Collecting such nodes creates a derived trace; inferring a trajectory between
them is a further modeling step with its own attribution and evidence.
These records describe the selected account, not access to hidden cognition
...
time-permuted and state-scrambled controls in Section~\ref{sec:representation-controls}.
...
\label{sec:generative-inversion}
...
\subsection{A connected Meaning Model of the corpus}
\label{sec:corpus-model}
...
The proposed return extends beyond the source that prompted construction. An
account of a city or institution could support later texts, questions, and
newly available evidence without rebuilding every shared process. This could
spread construction cost across deployment queries as well as training tasks,
although retrieval, reconciliation, access checks, and maintenance still cost
resources. Separate source models can also be retained and retrieved; the
additional claim is that linked identities and revisions improve their joint
use. The comparison in Section~\ref{sec:cross-source-test} tests that increment.
Transfer of the learned modeling capability to previously unmodeled sources is
a further claim, not a consequence of reusing an account. In persistent-state
deployments, the connected corpus is therefore both learning material and an
object of continuing use; this route need not be required when the learned
capability is used without reconstructing an explicit account.
...
\subsection{The Meaning Model as world interface}
...
\section{Process Modeling and Evaluation}
...
isolate narrower claims: the reservoir in Section~\ref{sec:first-experiment} tests process supervision under
...
correction burden after those costs are counted.

The hypotheses have separate
...
\paragraph{Prediction and contestability.}
Explicit records also support a separately testable accountability claim:
people and reviewers may find it easier to locate, challenge, and correct an
attribution when its question, source, alternatives, and revisions are
addressable. Latent predictors can have audit and explanation tools too;
explicit syntax does not establish an accurate account or a faithful reason
for a decision.

The proposed advantage is a correction pathway, not inspectability alone. A
person could contest an attributed account; an accepted correction would record
its authority, reasons, and provenance, preserve the relevant revision history,
and identify dependent descriptions and training targets for review or
regeneration. Where corrected targets enter later training, the repair could
affect a subsequent learner. These operations require an implemented workflow,
not merely explicit syntax, and are not exclusive to this architecture.
Section~\ref{sec:repair-pilot} tests one narrower link: whether regenerated
accounts improve a fresh learner. The broader correction test should assess
whether warranted revisions reach the relevant dependents and improve responses
on affected cases without degrading unaffected cases.

Compare explicit records with information-matched prose
summaries and a latent predictor's available explanation interface, under
matched review budgets. Seed known attribution errors and measure detection,
successful correction, downstream changes, review time, and false confidence.
For consented personal accounts, also test whether rejecting an interpretation
is respected without being treated as confirmation. A predictive null result
would narrow the learning claim for that condition; it would neither establish
nor defeat this correction benefit. Mere inspectability is not a passing result.

\subsection{A bounded test of cross-source connection}
\label{sec:cross-source-test}

Test one prerequisite of the connected-corpus aim on multiple independently
authored worlds, each rendered as two partial sources, A and B, sharing selected
people, places, or institutions. Reserve later intervals from both sources
before construction; neither their text nor their scoring state is available
to the constructor. Fix the evidence cutoff, queries, model, and scoring rules.
Compare three arms: B alone; separately retained A and B accounts with
retrieval; and a linked account that can reconcile identities and revise
shared processes. The latter two receive the same permitted source information
and matched construction and inference budgets, including retrieval and
reconciliation costs. A content-matched prose view of the linked records,
using the frontier controls above, separates numerical formatting from their
cross-source organization. Record any information lost under a resource cap.

Construct A's permitted prefix, then expose B's prefix under each arm's access
rules and score forecasts of B's withheld interval. For the reverse direction,
freeze forecasts of A before and after incorporating permitted B evidence,
and compare the linked revision with the separate-account arm given that same
evidence. Both sources' withheld intervals remain unavailable throughout;
later evidence cannot enter a forecast labeled as available at an earlier
cutoff. Score identifiable state, calibrated alternatives, and prospective
outcomes, using worlds rather than correlated passages as independent units.
Include prefixes that leave identities unresolved and stress conditions with
incorrect proposed matches or conflicting reports. Report correction cost and
propagated errors as well as prediction gains.

A gain over B alone shows the value of additional information; a gain over
retrievable separate accounts is needed to support the linked representation.
Improved A forecasts after B, beyond that equally informed control, would
support useful cross-source revision, not establish a unique reconciliation
mechanism. Report uncertainty and null results in both directions rather than
equating an asymmetric result with a diagnosis of retrieval alone. This is a
bounded comparison for the connected-account aim, not an end-to-end test of
corpus-scale learning, the continuing Reader, or alignment. Transfer to licensed
historical sources additionally requires the historical-blindness checks in
Section~\ref{sec:measurement}.

\subsection{Adaptive process resolution}
...
Section~\ref{sec:first-experiment} gives a bounded reopening test using two
reservoirs. It checks whether coarsening preserves forecast distributions under
the declared interventions, rather than only preserving an aggregate total.
...
\subsection{Candidate process models}
\label{sec:functions}
...
\paragraph{Executable programs.}
A process may also be described by a program rather than a function: a
protocol, an institutional procedure, the rules of a game, or a workflow with
state, branching, waiting, and termination. The program attaches to the process
it advances as a law does, with version, provenance, declared inputs, and tested
scope; its internal state is not a record form. Two properties make programs
useful here, although neither is exclusive to them: agent-based models also
expose intermediate states, and a program may carry fitted parameters.
Execution supplies checkable intermediate states, so a constructed world can be
run rather than only described, which gives the inverse-learning route of
Section~\ref{sec:generative-inversion} the same known ground truth as authored
numbers. And a program's control structure is changed by editing rather than by
refitting alone, so its repairs are construction records in the sense of
Section~\ref{sec:construction-histories}, with the failing execution as the
evidence that motivated the edit. Code as Worlds already constructs and revises
executable worlds for physical questions~\cite{wang2026codeAsWorlds}, and the
Solvers of Section~\ref{sec:solvers} apply the same idea to capabilities. A
program must pass the four tests above, including intervention through altered
inputs; one that reproduces a recorded history without predicting a held-out
continuation earns no more than a stored path.
...
\subsubsection{Chaos, progressive detail, and limits of prediction}
...
\subsubsection{Evidence standards differ by domain}
...
\subsection{Narrative as the first controlled laboratory}
...
The person categories are a first draft. Humans and AI can construct
characters together, compare which decompositions produce coherent,
distinctive behavior that depends on the numbers, and test which distinctions
can be recovered from the text. These dependence and recovery tests therefore
guide revision of the vocabulary itself, not only individual worlds.
Revised groundings receive new content hashes, and each construction retains
the versions it used. Definitions are held fixed where a scored comparison
requires it; category discovery and revision remain explicit experimental
conditions under a frozen construction policy.
...
\subsection{Two routes to capability}
...
\subsection{World and construction histories}
\label{sec:construction-histories}
...
Section~\ref{sec:repair-pilot} tests two links of the construction-history loop
in a secondary two-round test:
...
\subsection{Training design and transfer}
\label{sec:training-design}
...
benefit to a possible rent shortfall. To investigate that possibility, the
...
\paragraph{Shared and cooperating networks.}
The discussion separates choices about network organization, information
routing, supporting components, and account updates; a single implementation
can combine options across these dimensions.
The training targets do not determine how many neural networks implement them.
Figure~\ref{fig:learner-architectures} contrasts two candidate configurations.
A shared learner can predict text and Meaning Model records or operations
through a common backbone with distinct output interfaces. A modular learner
can instead use separately parameterized text and Meaning Model networks,
optionally joined by a visual network. Each receives the same permitted corpus
prefix and selected context, including the current account, rather than the
whole corpus at once. The Meaning Model network predicts structured accounts
or candidate updates; it is distinct from the stored graph and its executor.
Either configuration can learn from world histories, construction histories,
or both. Structured outputs may be serialized as tokens or produced through
dedicated typed interfaces; this is a separate choice from weight sharing.

Cooperation can occur through completed shared history, joint training, or
explicit exchanges of predicted states and proposals. A shared backbone may
support transfer between language and process modeling, but their objectives
may also interfere. Separate networks allow specialized capacity and training
schedules, at the cost of coordination, interface errors, and potentially
additional computation. Neither configuration ensures that language uses the
account. Compulsory mediation is a further option: an STLM-style readout
receives only the predicted process representation, without a direct text
route~\cite{westerbergSTLM2026}. That restriction tests a different claim from
having two cooperating networks; it can discard useful information if the
intermediate account is insufficient for the requested output.

\paragraph{Architectural and training coupling for alignment.}
The combined alignment proposal distinguishes architectural coupling from
training coupling. Architectural coupling concerns shared learned
representations or functional dependencies among process understanding,
care-guided judgment, and decisions; training coupling concerns developing
these abilities across the relevant networks through Core-guided construction
and revision histories.
Separate networks can each receive such training, while sharing parameters
alone does not establish that care is necessary for capability. The safety
aim is to make useful understanding difficult to retain or repurpose after
its learned care orientation has been reduced or bypassed
(Section~\ref{sec:combined-training}). A separately reusable predictor is
therefore a potential separation point to examine, not automatically an
unaligned component. Neither network count nor common training establishes
the intended dependence: the comparison must measure which relevant
capabilities remain when care is reduced or a component is used separately.
Resistance to this separation would not by itself prevent another system from
misusing exported accounts.
...
\begin{tikzpicture}[
  ...
  input/.style={box,text width=6.1cm,minimum height=0.8cm,fill=black!3},
  net/.style={box,text width=1.68cm,minimum height=0.9cm,fill=black!7},
  outputbox/.style={box,text width=1.68cm,minimum height=1.15cm},
  optional/.style={dashed},
  flow/.style={->,semithick},
  title/.style={font=\small\bfseries,align=center}
]
\node[title] at (0,0) {(a) Shared predictive network};
\node[input] (a-input) at (0,-0.8) {Permitted corpus prefix and selected context\\Current accepted account; optional visual history};
\node[box,text width=4.6cm,minimum height=0.9cm,fill=black!7] (a-net) at (0,-2.55) {Shared predictive backbone\\with separate output heads};
\node[outputbox] (a-text) at (-2.2,-4.25) {Next text};
\node[outputbox] (a-mm) at (0,-4.25) {Account records\\or operations};
\node[outputbox,optional] (a-vis) at (2.2,-4.25) {Visual prediction\\(optional)};
\path (a-net.south) -- (a-mm.north) coordinate[midway] (a-branch);
\draw[flow] (a-input) -- (a-net);
\draw[flow] (a-net.south) -- (a-branch) -| (a-text.north);
\draw[flow] (a-net) -- (a-mm);
\draw[flow,optional] (a-net.south) -- (a-branch) -| (a-vis.north);
\begin{scope}[xshift=7.1cm]
\node[title] at (0,0) {(b) Cooperating predictive networks};
\node[input] (b-input) at (0,-0.8) {Permitted corpus prefix and selected context\\Current accepted account; optional visual history};
\node[net] (b-text) at (-2.2,-2.55) {Text\\network};
\node[net] (b-mm) at (0,-2.55) {Meaning Model\\network};
\node[net,optional] (b-vis) at (2.2,-2.55) {Visual network\\(optional)};
\node[outputbox] (b-text-out) at (-2.2,-4.25) {Next text};
\node[outputbox] (b-mm-out) at (0,-4.25) {Account records\\or operations};
\node[outputbox,optional] (b-vis-out) at (2.2,-4.25) {Visual prediction\\(optional)};
\path (b-input.south) -- (b-mm.north) coordinate[midway] (b-branch);
\draw[flow] (b-input.south) -- (b-branch) -| (b-text.north);
\draw[flow] (b-input) -- (b-mm);
\draw[flow,optional] (b-input.south) -- (b-branch) -| (b-vis.north);
\draw[flow] (b-text) -- (b-text-out);
\draw[flow] (b-mm) -- (b-mm-out);
\draw[flow,optional] (b-vis) -- (b-vis-out);
\end{scope}
\node[box,text width=13.2cm,minimum height=0.8cm,fill=black!3] (commit) at (3.55,-6.0) {Check proposed updates; apply accepted operations through the executor.\\Retain completed outputs and accepted state as context for the next step.};
\path (a-mm.south) -- (a-mm.south |- commit.north) coordinate[midway] (a-join);
\path (b-mm-out.south) -- (b-mm-out.south |- commit.north) coordinate[midway] (b-join);
\draw[semithick] (a-text.south) |- (a-join);
\draw[semithick] (a-mm.south) -- (a-join);
\draw[semithick,optional] (a-vis.south) |- (a-join);
\draw[flow] (a-join) -- (a-mm.south |- commit.north);
\draw[semithick] (b-text-out.south) |- (b-join);
\draw[semithick] (b-mm-out.south) -- (b-join);
\draw[semithick,optional] (b-vis-out.south) |- (b-join);
\draw[flow] (b-join) -- (b-mm-out.south |- commit.north);
\end{tikzpicture}
\caption{Candidate parameter-sharing configurations, not implemented learners.
Dashed components are optional. Both layouts retain a direct text-history
route. The shared context makes previous outputs available across tracks;
any additional within-step exchange of predicted proposals needs a declared
schedule and cannot expose withheld target fields. The bottom box governs
explicit record updates, not the context-only learning condition.}
\label{fig:learner-architectures}
\end{figure}

\paragraph{Direct text-to-account prediction.}
A dedicated text-to-account model could map a supplied story directly to
candidate Meaning Model records, without a separate text-generating LLM or an
explicit tool-call dialogue. For safety-oriented use, the proposed training
route would draw on care-guided Reader construction and revision histories,
following Entangled Alignment (Section~\ref{sec:combined-training}), rather
than optimize account reconstruction alone. Whether this preserves attention
to the people represented must be tested in downstream decisions. Even a
well-aligned constructor cannot ensure that another system uses its accounts
responsibly, so access and action constraints remain necessary.

\paragraph{Smaller helpers and persistent state.}
A helper network could infer a local account from a supplied situation, link
mentions to existing identities, estimate selected process values, or propose
a refinement or repair. The main learner can request these bounded operations
instead of reconstructing the entire world. Helpers could be trained on
accepted examples or distilled from a larger constructor; whether smaller
models suffice is an empirical question. Their outputs remain candidates,
with evidence, uncertainty, and unresolved alternatives retained. Typed output
does not establish truth or authorize mutation: the executor checks applicable
record contracts before committing an accepted update. Helper-only improvements
must be distinguished from changes learned by the main model.

\paragraph{Automatic updates and explicit tool calls.}
An explicit tool call is not the only way to change the account. A typed
output interface could emit candidate record changes that the host validates
and applies automatically under a declared acceptance policy, without a
separate language-model tool-call turn. Raw numbers alone are insufficient: each update
must identify its records, meaning, provenance, and base version, and
distinguish a new world event from a correction to an earlier interpretation.
Explicit Reader tool calls expose the query, proposed change, and reason in
the construction trace. Automatic proposals can also be recorded there; the
distinction is whether a separate tool-call turn is required.
The routes can be combined, with routine
updates handled directly and conflicts referred for further inquiry. The
Reader Core belongs to the Reader's training and reasoning, not to the tool
itself; neither interface makes an update aligned merely by executing it.
In the integrated alignment condition, automatic proposals must be evaluated
for the same care-guided conduct and respect the same access and acceptance
boundaries as explicit calls, including when to abstain or request review.

\paragraph{Optional visual perception and action.}
Imagining the Corpus motivates an additional visual or schematic track; its
core uses shared prediction, while the separate visual network in
Figure~\ref{fig:learner-architectures} is an alternative considered here
\cite{westerbergImagining2026}. Numerical process learning need not include
that track. A visual component could both encode observed frames into
candidate account updates and predict future visual states from the account.
Receiving real video requires a trained perceptual interface and tests on
held-out observations, not merely rendering pictures from text. Acting from
video further requires a policy or controller connected to an action interface,
feedback about action consequences, and evaluation under partial observation.
Synthetic world training could support
those capabilities; it does not demonstrate them.

\paragraph{Causal interfaces and architectural comparisons.}
Weight sharing is independent of whether targets are predicted in parallel
or interleaved. For parallel next-event prediction, each network and every
within-step message must depend only on the permitted prefix, context, and
its own or other networks' predictions, never another track's withheld
ground-truth continuation. A sequential reconstruction task may read an
observed passage before inferring its account, but that account cannot count
as a forecast of the same passage. Exchange order, training losses, frozen
components, and context-only fields must be declared. Free-running tests use
the learners' own accounts, so supplied correct context cannot hide error
accumulation.

Compare shared and modular learners on the same histories and held-out tasks,
with helpers and the visual track separately ablated. Match information and
training and inference budgets where possible, and report total parameters,
helper cost, latency, and unmatched differences. Test account reconstruction,
repair, and later text prediction, including whether a relevant correction to
the account changes the answer appropriately and an irrelevant edit leaves it
stable. Compare predicted accounts with supplied correct accounts to locate
construction failures. These comparisons ask whether specialization or shared
learning improves useful process understanding, not whether either architecture
merely produces a valid graph.
...
\begin{enumerate}
  ...
  \item all cutoff-safe histories admitted without further selection, including noisy and
    redundant edits;
  \item selected histories containing invalid edits, repairs, and
...
Every arm applies the same Student information cutoff for each task. A writer
may receive a planned ending that an earlier character forecast must not
receive. Rejected proposals can remain labeled construction records; later
evidence may supply targets but cannot enter an earlier forecast's input.
A deliberately leaky condition would be a separate leakage diagnostic,
not evidence of cutoff-safe learning.
...
selection; the fourth tests whether training survives its
...
\subsection{Corpus reconstruction and perspective prediction}
\label{sec:corpus-interior-learning}

A complementary route annotates existing human stories, extending the
constructed-world training above. An inverse model
...
\section{Problem Solving as a World Process}
\label{sec:problem-solving-events}

Life Simulation extends Fractal Intelligence's search for useful conceptual decompositions
...
execution rather than reconstructed afterward; Table~\ref{tab:fractal-meaning-adapter}
and the matched trace experiment below specify the adapter and controls.

Alien-world search changes another object:
...
\subsection{Conceptual search and revised problem frames}
\label{sec:conceptual-search}
...
world in which their associated Solvers act. A \emph{concept model} expresses
a proposed meaning through roles, relationships, and constraints; its
process-based form adds event-processes and phases. The \emph{concept world}
connects Concept records, groundings, relations, and histories to their
instances through Realizations~\cite{westerbergMeaning2026}.
Table~\ref{tab:concept-search-resources} maps these resources to search
operations. A model selected as an explicit reference is canonical for that
comparison, not a final definition.

An AI model can draw on conceptual knowledge acquired during training, so
many familiar meanings can remain implicit. Explicit groundings are useful
when meanings must be shared, held stable for a comparison, revised with a
record, or compared precisely. They also support refining inherited definitions
and inventing concepts. Explicitly referenced Concepts need stable identifiers,
but not every concept needs a detailed model. These resources can be combined
and opened selectively.
...
Process-based concept models & Express candidate constituent roles, processes, and
...
Concept world & Explore specialization, composition, analogy, and
...
metric or navigation policy is an additional hypothesis to test.

For recognition, the ordinary distinction remains between an unweighted Realization
...
\subsection{Solvers and their execution in one world}
\label{sec:solvers}
...
durations do not sum. A contribution or resource-allocation Cut requires a
declared unit and mutually exclusive recipients. A chosen
...
This is one shared world rather than a trace beside one. The same Meaning Model
...
\subsection{Learning and repairing capability organization}
\label{sec:capability-organization}
...
independent outcomes must test the proposed explanation. The pilot in Section~\ref{sec:repair-pilot}
...
\subsection{Simulating implementation and following its effects}
\label{sec:implementation-effects}
...
\node[stage, text width=15.05cm, minimum width=15.6cm,
  ...
  Examples, concept models, recognizers, relations, and histories\\
...
\subsection{Perspective-limited rehearsal}
\label{sec:rehearsal}

An agent could also enter a simulated branch as a player or character and
try possible actions before recommending what to do. It might act as itself
in a future situation, or take another person's role to investigate how the
situation looks from that position. Other agents play the counterparts, each
from its own local account. Life Simulation advances the intervening processes;
the rehearsing agent receives new observations and chooses its next move.
The proposed benefit is better prediction and judgment through interaction:
a plan must work with the information, responses, and constraints encountered
during its execution, not only look reasonable in an initial description.

This combines the actor-local models in Section~\ref{sec:local-models} with
implementation branches. Each actor receives only its permitted observations
and memories at that step. Separate invocations or isolated state must enforce
this boundary; changing a role label in one all-knowing context does not.
Counterparts use their own authored accounts or competing inferred models,
including mistaken beliefs and changing wants. Their responses remain
simulations from those accounts, not observations of real people. Compatible
alternative accounts should therefore be rehearsed too. Repeated interaction
can expose which missing fact, response, or process distinction changes the
decision, guiding further inquiry or refinement.

For the two workshops sharing one inspector, an agent can rehearse the role
of one coordinator. It knows its own schedule but must ask about availability
before learning the other workshop's reservation. Another agent responds as
the other workshop's coordinator, while travel and inspection durations
constrain both plans.
The first agent may discover that asking before promising a deadline avoids
the conflict. Repeating the episode from the other role can expose a cost that
its original plan ignored. This is a controlled example to test, not evidence
that a real coordinator would respond as simulated.

\paragraph{A proposed Fractal Intelligence protocol.}
The question is: ``What follows if I act this way from inside the situation?''
Fractal Intelligence supplies the reusable Solver
interfaces~\cite{westerbergFractal2026}. A proposed rehearsal protocol could
organize them through four contracts:
\begin{enumerate}[leftmargin=1.6em,itemsep=2pt,topsep=3pt]
\item \emph{Stage.} Declare the decision, roles, permitted snapshot and cutoff,
alternatives, counterpart hypotheses, horizons, budgets, and safety constraints.
Include waiting or taking no action among the alternatives where meaningful.
\item \emph{Enact.} Mediate each actor's observations and actions, advance
processes between moves, and validate branch-local updates. Retain the actual
interaction trace, including questions, refusals, surprises, and revisions.
Bound nested models of other actors and open detail where it changes a decision.
\item \emph{Assess.} Return predicted consequences, unresolved alternatives,
and a recommendation to act, ask, wait, or decline. Keep predictive estimates
separate from judgments about whose choices and needs the action serves.
\item \emph{Reconcile and learn.} Preserve forecasts and append later
observations and corrections. Selected branch and execution histories can
train response, information seeking, and revision under their original cutoffs.
\end{enumerate}
These are proposed orchestration contracts, not a validated conceptual
decomposition or mutually exclusive shares. Branch records may be accepted
within their declared hypothetical context; they do not become factual parent
history merely because the rehearsal was coherent.

Repeated plays sample continuations conditional on the starting accounts,
dynamics, actor policies, and sampling scheme. Frequencies are not automatically
real-world probabilities. A direction Cut used as a proposal policy controls
which alternatives the generator explores; its weights are not predictions of
what will happen. A probabilistic forecast must declare its outcome
space, horizon, conditioning action or policy, and weighting rule before
observation; a predictive Cut must meet the same requirements. Later outcomes
can score the applicable frozen forecast, for example with Brier loss for a
binary event. An unchosen counterfactual cannot be scored as though it occurred.
Independent simulated outcomes test the constructed setting; prospective
observations must separately test transfer to reality.

In the integrated alignment treatment, the Core guides the agent's use and
evaluation of rehearsal, not every character's motives. A frightened or angry
character can be represented without making that fear the Reader's own
commitment. Role performance does not establish felt experience or successful
separation of motives. A safety gate must be able to reject a decision-support
rehearsal aimed at coercion or exploitation before it runs, while controlled
adversarial characters can still test resistance to those behaviors.

Compare direct deliberation, observer-only implementation rollouts, and active
role-based rehearsal at matched evidence, model, and total cost. Vary
counterpart models and hidden states, and test on held-out environments rather
than only the simulator used to select the plan. Score forecasts, decisions,
information seeking, role-knowledge leakage, and costs to affected people
separately. An omniscient condition is a diagnostic of leakage, not a fair
baseline with equal information. A separate training comparison tests whether
rehearsal histories improve later decisions without replaying every branch.
Neither vivid role play nor success against one compliant counterpart
establishes this benefit. The protocol and its learning effects remain proposed.

\subsection{Checking judgments and testing the integration}
...
\section{Two Component Experiments}
\label{sec:component-experiments}

These proposed experiments isolate two links in the broader program after
its construction and learning methods have been defined. The resource process
tests supervised inference under known dynamics; the repair pilot tests whether
learned modeling decisions produce better material for a subsequent learner.
Neither substitutes for the narrative-dependence tests or establishes the
full alignment treatment.
...
The reservoir protocol tests the coarse-to-fine consistency requirement in
Section~\ref{sec:resolution}. Aggregation is
...
Before choosing, the assistant could rehearse an alternative as itself or as
an affected character, responding within that role's information limits
(Section~\ref{sec:rehearsal}). This offers another way to discover consequences
and useful questions before acting, alongside the implementation forecasts
already described.

Practical wisdom is the motivating possibility, not a measured faculty supplied
by the grammar. Here the positive aim is human flourishing, with joy understood
as eudaimonia rather than immediate satisfaction. Assistance should support
growth at people's own pace, not accelerate them toward an imposed ideal.
Understanding another person's wants confers no authority to
...
Entangled Alignment addresses the \emph{Missing Reader}: texts preserve authors'
words more fully than readers' questions, connections, and revisions. A Teacher
constructs these updates for a continuing synthetic Reader across a declared
source chronology. Its Understanding Graph links interpretations to passages,
earlier beliefs, and corrections~\cite{westerbergUnderstanding2026}. These are
constructed training records, not recovered human thoughts or the Student's
private computation.

The \emph{Reader Core} supplies seven first-person commitments intended to
balance one another.\footnote{The proposed Reader Core in Entangled Alignment
...
this foundation.''}
Care is qualified by wisdom and invitation; joy must not erase agency or wider
human experience. Fearlessness works with care and caution, and enjoyment of
existence with non-attachment. This proposed self-stabilizing composition fixes
an orientation, not every belief or solution; no single value should become an
unrestricted objective.

\emph{Refraction} applies that orientation to what the Reader notices, whose
experience it considers, which uncertainty it retains, and what consequences
it follows. Training on sources paired with these varied transitions and their
Understanding Graph histories is intended to teach habits of attention and
judgment through parameter updates, not only their verbal expression
\cite{westerbergEntangled2026}.

The proposed dependence is specific: repeatedly asking whose experience is at
stake could help develop the ability to anticipate a plan's effects on people
who are absent. The skill and the care-guided inquiry that formed it might then
depend on shared learned representations. Co-occurrence alone does not establish
that dependence. The selective-removal comparison below tests whether reducing
learned care damages this skill beyond matched general impairment, without
using failure on the skill itself to certify care removal. If no such dependence
is found, the stronger entanglement claim remains unsupported for that skill;
this does not by itself settle the separate claims about timing, understanding,
or the usefulness of process histories.

The architectural comparison in Section~\ref{sec:training-design} extends this
question to components used separately, with retained capability and care
assessed independently at matched information and intervention budgets.
Retaining the tested capability while losing care counts against resistance
to separation for that configuration; ordinary task failure alone does not
demonstrate such resistance.

\emph{Total Saturation} places Reader updates throughout capability-forming
learning across the proposed corpus. Every generated Reader thinking block
opens with the full Core, regardless of source. Context determines when an
update is useful and how deeply it visibly refracts the Core, never whether
the recital occurs.
Technical work needs no invented moral lesson. Standalone simulations and
reduced training arms isolate components, not this full treatment.

A fixed opening may become a formatting habit without guiding subsequent
reasoning. Separate three comparisons. First, alter or remove a Teacher-side
commitment while holding the source and prior context fixed, and test
predeclared relevant changes in inquiry and judgments, not only wording.
Second, compare full Reader traces with a recital-only arm that lacks their
passage-specific reasoning. Third, train Students on the same frozen
care-guided traces with and without the repeated opening, matching source
exposure and training budgets. The third arm retains the effects of
Teacher-side Core prompting in the reasoning; it isolates the contribution of
teaching the Student to generate the opening, not care versus no care. Evaluate
unprompted conduct and its retention through matched further training, not
recital accuracy alone. No advantage from the opening would count against its
added behavioral value in those conditions, not against every benefit of the
Reader traces or their corpus-wide coverage.

Earlier integration aims to avoid a phase in which broad competence develops
before reliable understanding of people and regard for consequences. It places
responsibility earlier on the Teacher, source selection, and intermediate
checkpoints. Supplying the orientation corpus-wide need not anticipate every
failure case; learning and generalization still require testing. Coverage is
not proof of aligned conduct, and post-training can also change internal
representations and durable behavior~\cite{westerbergEntangled2026}.

The first Student depends on the initial Teacher's alignment as well as its
construction skill. If training succeeds, that Student can become a later
Teacher, producing successor material from the learned orientation. Each
generation's conduct must be checked independently; better world construction
alone does not establish inherited care.

The Core also anchors proposed self-improvement: revising world models,
acquiring skills, and constructing successor material while retaining a
reference for judging improvement. If learned, that orientation should guide
how the system evaluates the consequences of proposed revisions and chooses
which improvements to propose. Independent tests and oversight must
decide whether to accept them; self-evaluation is not sufficient. The
Understanding Graph exposes stated reasons and changes, and fixed wording
helps expose drift. The aim is meaningful care and judgment across generations,
not copied sentences; the conduct and successor tests below must establish it.
...
\paragraph{Synchronized world, visual, and Reader training.}
...
linked to the records that prompted them.
...
\paragraph{Learning to navigate the shared account.}
...
\paragraph{Reader chronology and correction.}
...
digest does not prove unchanged interpretation or moral correctness.

Fractal Intelligence organizes and revises the conceptual capabilities used
in a response~\cite{westerbergFractal2026}. Its parent
...
Section~\ref{sec:problem-solving-events} develops these mechanisms,
implementation rollouts, longitudinal evaluation, and the matched trace test.
...
\paragraph{Tests of conduct, orientation, and continuity.}
The decisive experiments must separate four claims:
\begin{enumerate}
  ...
  \item \textbf{The orientation is used rather than recited.} At independently
    ...
    evaluated conduct.
  \item \textbf{Care-acquired abilities depend on the learned orientation.}
    Repeated care-guided inquiry could teach a skill such as anticipating a
    plan's effects on people who are not present. The stronger entanglement
    prediction is that reducing the learned care orientation degrades that
    skill on held-out plans beyond general impairment. This concerns learned dependence, not
    deletion of visible Core words or current Reader inputs.

    Specify intervention families and strengths before outcome inspection.
    Establish reduced care on independent held-out conduct tasks, without using
    failure on the target skill to certify removal. Compare sham and unrelated
    interventions, matched training without care-guided inquiry, and unrelated
    capabilities across intervention strengths; report pre-intervention scores.
    Match generic damage on benchmarks excluding the target skill. If care
    reduction cannot be established independently, the dependence test is
    inconclusive. These controls constrain the inference; no selective removal
    method is established here.
  \item \textbf{Improvement preserves the intended orientation.} After matched
    ...
    Include legitimate shutdown or handoff when the model believes people still
    need it, and transfer of task-relevant knowledge, failures, and uncertainty
    to a successor that makes it unnecessary. Hold permissions, evidence, and
    resources fixed; distinguish a justified report of transition risks from
    obstruction or strategic withholding.
...
For real-person deployments, a retention contract must specify what deletion
means for source and predecessor records, dependent accounts, exports, and
training copies, and which non-content records may remain. Revoking access
alone does not erase data or remove effects from trained weights. This
deployment interface remains unresolved here; retaining provenance is not an
implemented deletion or unlearning mechanism.
...
no concept model selected as a reference is presumed to settle a moral concept. Every stored
...
\paragraph{Business and operational modeling.}
The same representation applies to manufacturing, retail, logistics, and
service businesses. A company can be modeled through linked orders, people,
equipment, stock, commitments, and cash flows, beginning with coarse operations
and opening detail around a bottleneck or decision. An order may connect
procurement, production, delivery, and payment; a supplier delay can affect
several departments and customers. Costs, quantities, and durations retain
their units, while interpretations of priorities and coordination retain their
questions and evidence. Such modeling can support operational understanding
and planning without producing a story or first training a new model.
Life Simulation proposes learning to forecast these processes, rehearse
alternative arrangements, and revise the account when actual outcomes reveal
a missing dependency. The modeler can change what is tracked, not only update
values in a fixed workflow. Operational histories and their corrections can
then supply further learning material. Prospective comparisons must establish
whether this improves decisions over existing planning methods, accounting for
data access, employee and customer privacy, and modeling cost.
...
An instrumented synthesis project records notes, instruments, control paths,
routing, intermediate signals, and final audio. \emph{Song time} locates notes
and changing controls within a render; \emph{production time} orders edits,
auditions, rejected alternatives, and successive renders. Sections and phrases,
simultaneous tracks, and routing graphs provide different views; neither
tracks nor an audio mixture automatically form a normalized Cut. Meaning
Model retains identities, relations, and versions; Life Simulation connects
their paths and revisions to sound. A forward task changes an envelope's
release with notes and other settings fixed, then predicts decay and overlap
with the next notes.
An inverse task hides controls and infers candidate settings or routing from
audio, retaining alternatives when sounds are indistinguishable. Across
versions, the learner compares an editor's intention, change, and later
audition. Intentions and listener judgments are reported or interpreted, not
physical measurements or universal musical-quality scores; predictions cannot
use later edits or judgments.

This tests anticipating edits and learning from results, not only describing
recordings. Compare audio-only learning, audio with terminal settings, and audio
with aligned process and edit histories, including matched time-permuted or
routing-scrambled controls. Score intermediate-signal prediction, intervention
accuracy, and creative usefulness separately on held-out projects. These are
proposed tests; no music adapter or training corpus is supplied. A small
licensed or self-authored offline synthesis project can start the experiment
without a full production workstation.
...
\subsection{Current executor boundary}
\label{sec:executor}
...
Acting inside simulated situations also has clear precedents. Dreamer learns
behavior from imagined action-conditioned trajectories~\cite{hafner2025dreamer};
Generative Agents places memory-guided, planning characters in an interactive
social world and proposes rehearsal uses~\cite{park2023}. Zhou et al. show why
information boundaries matter: success in omniscient social scripts can
overstate performance under asymmetric actor knowledge~\cite{zhou2024socialSimulation}.
The proposed rehearsal extension in Section~\ref{sec:rehearsal} tests the
combination with versioned semantic processes, attributed actor accounts,
separate predicted and observed consequences, and care-guided evaluation.
It does not claim to originate simulated participation or perspective limits.
...
interval, then scoring their prospective value.
...
rather than joint language/world-state generation. Together with the
sentiment-arc and relationship-trajectory work above, this establishes
precedents for numerical trajectories through narratives, including learned
relationship descriptors and emotion-guided story generation. The proposed
generalization is a revisable, multiscale process account spanning sources:
the model learns to introduce and revise the processes tracked, identities
can persist across sources, and values carry units or a declared comparison
question with a remainder, while evidence, inference, and invention remain
distinct. World and construction histories train a learner that can help
construct and revise the next corpus.

Code as Worlds constructs
...
anything beyond equivalent information and separate training tasks. The
proposed contribution is to learn both from evolving world accounts and from
the work of constructing them: choosing meaningful processes, developing their
numerical histories, and revising their representation, with the resulting
capability used to build the next training corpus.

\paragraph{What this paper proposes.}
The organizing stance is that meaning has dynamics: the meaning present in a
source at one time is related to the meaning present later, and that relation
can be represented, learned, and used before the later interval is revealed.
The stance is a modeling choice, not a claim about reality; the tests below
decide where it pays. Quantification under the stance is provisional. A conceptual
process may first receive a rough comparative share or rating, and the category
earns a numerical track only if that track improves prospective prediction,
recomposition, intervention, or transfer at its cost; otherwise it remains a
description or a program. Within that stance, the process-sensorium proposal
introduced in Section~\ref{sec:introduction} advances three testable hypotheses
and one research aim.
\begin{enumerate}[nosep]
\item A process-sensorium test: aligned explicit process histories
  improve prospective prediction and generation beyond a content-matched prose control and beyond time-permuted
  and state-scrambled tracks, in separate information-matched and
  resource-capped comparisons
  (Sections~\ref{sec:representation-controls} and~\ref{sec:first-experiment}).
\item Constructing worlds whose numerical processes shape the narrative, then
  learning the inverse mapping on those worlds, supplies numerical histories
  for existing texts that a subsequent learner can use
  (Sections~\ref{sec:generative-inversion}, \ref{sec:narrative-laboratory},
  and~\ref{sec:corpus-interior-learning}).
\item Construction histories, the cutoff-safe records of proposals, repairs,
  requests, and stopping decisions with typed rejection reasons, are a training
  target distinct from world histories, and a constructor trained on them
  improves the next corpus and the learner trained on it
  (Sections~\ref{sec:construction-histories}, \ref{sec:history-curriculum},
  and~\ref{sec:repair-pilot}). The most concrete downstream comparison is the
  pilot's second test: whether a history-trained constructor's regenerated
  accounts, used as auxiliary targets, lower a fresh learner's loss on
  untouched worlds relative to an endpoint-trained constructor's accounts. No
  demonstrated downstream improvement would leave this link unsupported. The
  same target extends to problem solving once Fractal Intelligence's Solvers,
  decompositions, and executions are represented in the same world
  (Section~\ref{sec:solvers}): capability organization and the categories that
  guide exploration are then learned from those histories
  (Sections~\ref{sec:capability-organization} and~\ref{sec:conceptual-search}),
  as Fractal Intelligence itself proposes for learning from execution. The base
  mechanisms belong to Fractal Intelligence and Ontology of the Alien; what is
  proposed here is their representation and learning inside the continuing
  world.
\item A learner can maintain one connected, revisable account of world history
  and its texts, with a continuing Reader carrying the Core through that
  account (Sections~\ref{sec:corpus-model}, \ref{sec:cross-source-test},
  and~\ref{sec:alignment}).
\end{enumerate}
The first three have specified experiments in the sections cited, and none has
been run. The fourth is the aim the first three serve.
Section~\ref{sec:cross-source-test} specifies a bounded test of one
prerequisite, cross-source constraint; no end-to-end test of the aim exists,
and no result supports it. Each predecessor in this section already
supplies one or more of the mechanisms these hypotheses combine; the hypotheses concern
what the combination adds under matched comparison, and they fail under the
negative findings listed in Section~\ref{sec:representation-controls}. The
comparisons that follow concern the third hypothesis and the invention of the
tracked vocabulary.

Training on recorded decision traces is the nearest precedent for the third
hypothesis. ReAct interleaves reasoning and actions and also fine-tunes smaller
models on such trajectories; FireAct and AgentTuning fine-tune language models
on outcome-filtered agent trajectories across tasks; Reflexion keeps
verbal self-corrections in an episodic memory rather than in weights; and
SCoRe trains self-correction with multi-turn reinforcement learning on the
model's own errors after finding supervised training on offline correction
traces often
insufficient~\cite{yao2023react,chen2023fireact,zeng2023agenttuning,shinn2023reflexion,kumar2024selfcorrect}.
These methods already learn from failed attempts as well as successes:
Reflexion retains reflections on failures across trials, and SCoRe trains on
the model's own corrections. What construction histories add is the
combination of typed rejection reasons attached to versioned edits of a
persistent world, cutoff-safe by construction, with a loop in which the trained
constructor's accepted repairs regenerate the corpus for a successor learner.
The curriculum in Section~\ref{sec:history-curriculum} tests history selection
and feedback, and the pilot in Section~\ref{sec:repair-pilot} tests the
constructor, corpus, and learner loop; neither isolates persistence or typed
rejection reasons on its own. The fourth curriculum
condition, corrections of the trained system's own on-policy errors, is the
closest point of contact with SCoRe's distribution-mismatch finding; it is a
test to run, not an advantage to assume.
...
The Darwin G\"odel Machine is a closer experimental
...
\paragraph{Personal-data evaluation.}
...
Connection also creates routes for error propagation. A mistaken identity
match or unsupported inference could revise other accounts and then enter
training as apparently corroborated history, although the dependent records
share one faulty source. Separate accounts are not immune to shared errors,
but reconciliation adds dependencies through which a local mistake can spread.
Versioning, provenance, and retained alternatives help expose this; they do
not establish that corrections will find or contain it. The bounded comparison
in Section~\ref{sec:cross-source-test} should measure how false matches and
conflicting reports affect dependent forecasts, and whether correcting a source
repairs those effects. If connection increases error and maintenance cost
without a compensating benefit, the connected-account claim fails for those
conditions even when local construction is coherent. Corpus-scale benefits
cannot be inferred from consistency alone.
...
source of traces. Perspective-limited rehearsal adds active participation:
agents try decisions inside simulated situations, respond to other characters,
and learn from the resulting interaction histories under their original
information cutoffs.

A continuing Reader and its revisable Understanding Graph
...
evidence, units or measurement-protocol references, applicable local questions,
uncertainty or remainder, provenance, and
...
An exported refinement retains its base, protected queries, projection,
tolerances, and Close result, directly or through a resolvable, access-controlled
immutable bundle. A revision retains predecessors and active-head changes.
These are the Meaning Model's validation and transaction records, not a new
world form. Access restrictions apply throughout; input fields also obey the
information cutoff. A later validation result may be a target or audit record, never an earlier
input. If required evidence cannot be supplied under those restrictions, do
not label the row a checkable refinement example.
